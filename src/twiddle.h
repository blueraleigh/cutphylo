...
*/
static int twiddle(int *x, int *y, int *z, int *p)
{
    register int i, j, k;
    j = 1;
    while (p[j] <= 0)
...
static void inittwiddle(int m, int n, int *p)
{
    int i;
    p[0] = n+1;
    for (i = 1; i != n-m+1; i++)
...
// Additions by MCG for use with option (1)

static int twiddle_begin(int m, int n, int *p, int *b)
{
  int i;
  for(i = 0; i != n-m; ++i)
    b[i] = 0;
...
static int twiddle_next(int *x, int *y, int *z, int *p, int *b)
{
  int notdone = !twiddle(x, y, z, p);
  if (notdone)
  {
    b[*x] = 1;
    b[*y] = 0;
...
/************************
  Here is a sample use of twiddle_begin() and twiddle_next():

int n = 5;
int m = 3;
int i, x, y, z, p[n+2], b[n];
for (i = twiddle_begin(m, n, p, b); i; i = twiddle_next(&x, &y, &z, p, b))
{
  // do something with b
}
